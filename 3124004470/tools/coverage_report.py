"""用标准库 ``trace`` 统计单元测试的语句覆盖率。

程序本身零第三方依赖，覆盖率只是开发期的度量手段，因此这里直接用标准库的
``trace`` 模块实现，任何只装了标准库的电脑都能复现这张覆盖率表格。

用法（在项目根目录执行）::

    python tools/coverage_report.py

输出：屏幕上的覆盖率汇总表 + ``docs/coverage/`` 下带 ``>>>>>>`` 标记的源码副本。
"""
...
REPORT_PATH = PROJECT_ROOT / "docs" / "coverage_report.txt"
...
def main() -> int:
    ...
    # 只关心本项目源码的覆盖率：标准库、Python 安装目录、测试与工具脚本一律忽略。
    ignoredirs = [
        sys.prefix,
        sys.exec_prefix,
    ...
    report_lines = ["语句覆盖率（只统计 quiz 包）：", ""]
    report_lines.append(f"{'模块':<26}{'覆盖率':>8}{'总语句':>8}{'未覆盖':>8}")
    report_lines.append("-" * 50)
    total_statement = 0
    total_hit = 0
    for line in buffer.getvalue().splitlines():
        if "quiz" not in line or not line.strip():
            continue
        fields = line.split()
        ...
        total_hit += hit
        report_lines.append(
            f"{module:<26}{percent:>8}{statements:>8}{statements - hit:>8}"
        )
    if total_statement:
        overall = total_hit / total_statement * 100
        report_lines.append("-" * 50)
        report_lines.append(
            f"{'合计':<26}{overall:>7.1f}%{total_statement:>8}"
            f"{total_statement - total_hit:>8}"
        )
